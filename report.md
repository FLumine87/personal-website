...
The website is built using Node.js with Express as the web framework and EJS for dynamic page rendering. Socket.IO enables real-time multiplayer functionality in the puzzle game. The project follows a standard server-side architecture with route handling, game logic, and views clearly separated.
...
*Figure 1: Website architecture and client-server data flow.*
...
**Game Rules and Scoring:** Players take turns placing blocks on a 4×4 grid. Each block has one of four shapes (circle, square, triangle, star) and four colors (red, blue, green, yellow), creating 16 unique types. Placing a block triggers line checks in four directions; matches of three or more (by shape or color) are cleared for 1 point each. A full board triggers a jackpot, clearing all blocks for 16 bonus points. Players are eliminated by 60-second timeout or disconnection; the game ends when all players leave.

**Data Structure:** The board uses a 16-element one-dimensional array (`board`), with `row * 4 + col` mapping 2D coordinates to indices. Each cell stores a block object `{id, shape, color}` or `null` if empty. The `availableBlocks` array manages the block pool for random selection during gameplay.
...
**Figure 3: Socket.IO Multiplayer Management**
...
*Figure 4: Sequence diagram showing Socket.IO event flow between clients and server.*
...
**UI Design and CSS Refinement.** For UI design, this is not a code problem but rather an aesthetic challenge. I drew reference from other excellent personal websites to achieve a clean and visually appealing page. However, integrating multiple strong visual designs introduces alignment issues and color inconsistency that require careful adjustment. Browser developer tools were used extensively for fine-tuning spacing, font sizes, and color values in real time. AI large language models were also employed to help identify and eliminate style redundancy and inconsistency, ensuring the final stylesheet remained clean and maintainable.
...
3. MDN Web Docs. "HTML5: The HyperText Markup Language." Available at: https://developer.mozilla.org/en-US/docs/Web/HTML (Accessed: May 2026)
4. Node.js Documentation. "Node.js v24.13.0 Documentation." Available at: https://nodejs.org/docs/ (Accessed: May 2026)
5. W3Schools. "CSS Tutorial." Available at: https://www.w3schools.com/css/ (Accessed: May 2026)
6. DeepSeek. (2025). DeepSeek [Large language model]. Used for fixing one CSS class name addition error in the code, specifically the issue where classList.add() cannot handle strings containing spaces.
